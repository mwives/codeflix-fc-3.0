...
describe('CategorySequelizeRepository', () => {
  ...
  describe('bulkInsert', () => {
    it('should insert multiple categories', async () => {
      ...
      expect(categoriesInserted[0].categoryId).toBe(categories[0].categoryId.id)
      expect(categoriesInserted[1].categoryId).toBe(categories[1].categoryId.id)
    })
  })
  ...
  describe('delete', () => {
    ...
    it('should throw an error when category is not found', async () => {
      ...
      await expect(repository.delete(categoryId)).rejects.toThrow(NotFoundError)
    })
  })
